<template>
  <div class="view-day-wrap">
    <BreadCrumb second_path="总体概览" third_path="周志查看" />
    <el-card>
      <!--提示-->
      <el-alert
              title="查看同部门其他人员的历史周志"
              type="warning"
        ...
        <el-col :span="21" class="right-col">
          ...
          <el-timeline>
            ...
            >
              <el-card>
                <el-tag>本周总结</el-tag>
                <p v-html="item.week_summery"></p>
                <el-tag>下周安排</el-tag>
                <p v-html="item.week_next_week"></p>
              </el-card>
            </el-timeline-item>
            <!--如果数据为空的时候-->
...
<script>
  export default {
    ...
    methods: {
      ...
      // 根据ID获取历史周志
      async getHistoryWeek(id) {
        const result = await this.$api.getHistoryWeek({ id });
        this.time_line_data = result.data;
      },
      // 人名的点击事件
      handleSelect(user_id) {
        // 根据ID获取历史日志
        this.getHistoryWeek(user_id);
      },
    },
    created() {
      // 获取部门人员
