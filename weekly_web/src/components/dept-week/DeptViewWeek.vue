<template>
  <div class="submission-wrap">
    <BreadCrumb second_path="部门周志" third_path="部门概览" />
    ...
    <el-card>
      ...
      <el-row :gutter="20">
        ...
        <el-col :span="6">
          ...
          ></el-progress>
          <div class="class-name">周志评阅情况</div>
          <div class="person-count">共有{{ day_person_count }}人</div>
        </el-col>
      </el-row>
    </el-card>
...
export default {
  ...
  methods: {
    // 获取日志提交情况
    async getDay() {
      const { data: result } = await this.$api.adminViewWeek();
      // 计算百分比
    ...
    // 获取日志评阅情况
    async getDayRead() {
      const { data: result } = await this.$api.adminReadWeek();
      // 计算百分比
      if (result[0].all_num === 0) {
        return;
      }
