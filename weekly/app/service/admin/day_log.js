...
class DayLog extends Service {
   ...
   * @param {number} dept_id 部门ID
   */
  async getTodayLogReadSameDept(date, dept_id) {
    const logs = await this.app.mysql.query(`select * from user,day_log where day_create_time like '%${date}%' and user.user_id = day_log.day_user_id and user.user_department_id = '${dept_id}' and user.user_is_admin = '0' and day_log.day_read_status = '1'`);
    return logs;
  }
  /**
   * 获取下级部门经理日志评阅数
   ...
   * @param {number} dept_id 部门ID
   */
  async getTodayLogReadDownDept(date, dept_id) {
    const logs = await this.app.mysql.query(`select * from user,day_log where day_create_time like '%${date}%' and user.user_id = day_log.day_user_id and user.user_up_department_id = '${dept_id}' and user.user_is_admin = '1' and day_log.day_read_status = '1'`);
    return logs;
  }
  /**
   * 根据用户ID获取历史日志
